...
#'
#' This function allows the user to manually locate the separators between
#' columns of a table in a pdf. The output can be used as the \code{columns}
#' argument in \code{extract_tables()}
#' 
#' Manually selecting the separators can ensure that values stay in their
#' respective columns. This is useful when some rows of a table have no or only
#' little white space between columns. The code is an adaptation of the
#' \code{locate_area()} function and its helpers.
#' 
#' @inheritParams locate_areas
#' @return For \code{locate_areas}, a list of four-element numeric vectors
